...
// This file is from learncpp.com, but is edited to my liking
...
namespace Random_number_generator
{
    ...
    inline std::mt19937 mersenne_twister { generate() }; // generates a seeded std::mt19937 and copies it into our global object

    // Generate a random int between [min, max] (inclusive)
    // * also handles cases where the two arguments have different types but can be converted to int
    inline int get(int min, int max)
    {
        return std::uniform_int_distribution { min, max }(mersenne_twister);
    }
    ...
    T get(T min, T max)
    {
        return std::uniform_int_distribution<T> { min, max }(mersenne_twister);
    }
